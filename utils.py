...
def eval_pr(y_pred, y, num):
    prec, recall = torch.zeros(num).cpu(), torch.zeros(num).cpu()
    thlist = torch.linspace(0, 1 - 1e-10, num).cpu()
    for i in range(num):
...
def f_measure(pred,gt):
    beta2 = 0.3
    with torch.no_grad():
        pred = torch.from_numpy(pred).float().cpu()
        gt = torch.from_numpy(gt).float().cpu()
