...
p = {}
p['datasets_path'] = './dataset/pothole600/'
p['train_datasets'] = [p['datasets_path'] + 'training', p['datasets_path'] + 'validation']
p['val_datasets'] = [p['datasets_path'] + 'testing']
...
p['snapshot_path'] = 'weights/[{}]_[{}]'.format(time.strftime('%Y-', time.localtime(time.time())), p['model'])
...
exec('from nn.{} import MyNet'.format(p['model']))
...
class Trainer(object):
    def __init__(self, p):
        ...
        shutil.copyfile(os.path.join('nn', p['model'] + '.py'), os.path.join(p['snapshot_path'], p['model'] + '.py'))
        ...
        self.train_set = RgbdSodDataset(datasets=p['train_datasets'], transform=transform_train, max_num=p['max_num'],
                                        if_memory=p['if_memory'])
        self.train_loader = DataLoader(self.train_set, batch_size=p['bs'], shuffle=True, num_workers=p['num_workers'],
                                       pin_memory=False)

        self.val_loaders = []
        for val_dataset in p['val_datasets']:
            val_set = RgbdSodDataset(val_dataset, transform=transform_val, max_num=p['max_num'],
                                     if_memory=p['if_memory'])
            self.val_loaders.append(DataLoader(val_set, batch_size=1, shuffle=False, pin_memory=False))

        self.model = MyNet()

        self.model = self.model.cpu()

        self.optimizer = utils.get_optimizer(p['optimizer'][0], self.model.get_train_params(lr=p['lr']),
                                             p['optimizer'][1])
